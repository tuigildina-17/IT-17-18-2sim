...
{
    if (!node) throw runtime_error("Дерево пустое"); // Проверка на пустое дерево
    while (node->right) // Идем вправо до самого последнего узла
    {
        node = node->right;
    }
    return node->value; // Возвращаем максимальное значение
}

// Метод для нахождения максимального значения в дереве с использованием корня
int BinarySearchTree::findMax() const 
{
    return findMax(root); // Вызываем метод нахождения максимума с корнем дерева
}
